# Phaser 3 - Space Shooter Game

![License](https://img.shields.io/badge/license-MIT-green)

A fun arcade space shooter built with [Phaser 3](https://github.com/photonstorm/phaser)!

This is the official code repository for the mosquito shooter game.

## How To Play

With a Keyboard

Move with right and left keys, shoot with spacebar.

## Credits

Followed from tutorial [Phaser 3 Game Tutorial - Space Shooter](https://www.youtube.com/playlist?list=PLmcXe0-sfoShHpVWLDW2gqsyH1UFRni_2)

This project would have not been possible without the use of some awesome assets created by some amazing artists! This project would not have been possible without the following people/resources:

Explosion Effects - Foozle - (https://foozlecc.itch.io)

Void - Main Ship (1.0)

    Commissioned from: Baldur (https://twitter.com/the__baldur)
    Distributed by Foozle (www.foozle.io)

    		------------------------------

    License: (Creative Commons Zero, CC0)
    http://creativecommons.org/publicdomain/zero/1.0/

    This content is free to use and modify for all projects, including commercial projects.
    Attribution not required.  If you would like to support, consider a voluntary donation.

    		------------------------------

    Donate:   https://foozlecc.itch.io/
    Patreon:  https://www.patreon.com/bePatron?u=48464594

    Follow on YouTube and Twitter for updates:
    https://www.youtube.com/c/FoozleCC
    http://twitter.com/FoozleCC
